...
class Database {
    ...
    public static function connect() {
        self::$con=@mysql_connect(Config::read("database", "host"),Config::read("database", "username"),Config::read("database", "password"));
        if(!self::$con) {
            Engine::logError("database", 400);
            return FALSE;
        }
        $selected=@mysql_select_db(Config::read("database","name"),self::$con);
        if(!$selected) {
