...
class HTMLHelper extends Helper {
    ...
    public static function getImage($path,$width='',$height='',$id='') {
        $html=sprintf('<img width="%s" height="%s" src="%s%s" id="%s" />',$width,$height,Engine::path("view"),$path,$id);
        return $html;
    }

    public static function getLoader($path,$type,$id){
        $html=sprintf('<div class="loader" id="%s"><img src="%s%sloader-%s.gif" /></div>',$id,Engine::path("view"),$path,$type);
        return $html;
    }
